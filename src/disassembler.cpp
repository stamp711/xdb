...
std::vector<disassembler::instruction> disassembler::disassemble(std::size_t n_instructions,
    ...
    auto code = process_->read_memory_without_traps(address.value(), n_instructions * MAX_LENGTH_OF_X86_64_INSTRUCTION);
    std::span<const std::byte> code_span(code.data(), code.size());

    ZydisDisassembledInstruction instr;
    while (n_instructions > 0 && !code_span.empty() &&
           ZYAN_SUCCESS(ZydisDisassembleIntel(ZYDIS_MACHINE_MODE_LONG_64, address->addr(), code_span.data(),
